...
```bash
bash ~/Documents/IWE/FMT-exocortex-template/roles/strategist/scripts/strategist.sh day-plan
```
...
Установка (в терминале): `bash ~/Documents/IWE/FMT-exocortex-template/roles/extractor/install.sh`
...
Установка (в терминале): `bash ~/Documents/IWE/FMT-exocortex-template/roles/synchronizer/install.sh`
...
```bash
cd ~/Documents/IWE/FMT-exocortex-template
...
**MEMORY.md не загружается** — проверь путь: `~/.claude/projects/-Users-<username>-Documents-IWE/memory/MEMORY.md`. Имя директории = путь к workspace через дефисы.

**DS-strategy не создан** — вручную: `mkdir -p ~/Documents/IWE/DS-strategy/{current,inbox,docs,archive} && cd ~/Documents/IWE/DS-strategy && git init`.
